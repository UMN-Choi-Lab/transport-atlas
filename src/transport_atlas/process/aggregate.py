...
def _venue_stats(papers: pd.DataFrame, venues: list[dict]) -> list[dict]:
    ...
    collaborations, papers_per_author, avg_citations.
    """
    ...
    # Pre-extract per-paper: venue, n_authors, author_keys, cites — single pass over papers
    rows = []
    for _, r in papers.iterrows():
        ...
        rows.append({
            ...
            "n_authors": len(uniq_keys),
            "author_keys": uniq_keys,
    ...
    for slug in venue_short:
        ...
        # Collaborations in a paper p = C(n_p, 2). Sum across papers.
